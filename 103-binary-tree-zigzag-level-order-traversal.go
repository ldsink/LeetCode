...
func zigzagLevelOrder(root *TreeNode) [][]int {
	...
	stack := []Node103{Node103{node: root, level: 0}}
	for ; len(stack) > 0; {
		...
		if node.Left != nil {
			stack = append(stack, Node103{node: node.Left, level: level + 1})
		}
		if node.Right != nil {
			stack = append(stack, Node103{node: node.Right, level: level + 1})
		}
	}
